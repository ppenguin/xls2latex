...
class xlWorkbookTeX(openpyxl.Workbook):
    ...
    def getTeX(self, sheetname, booktabs=True, tabular=True, longtable=True, caption=None, label=None):
        ...
        ## get some basic dimension data we always need (just column widths for now)
        twidth = 0.0
        colwidth = []
        ...
        for c in table_tuple[0]:
            twidth += sheet.column_dimensions[c.coordinate].width
            colwidth.append(sheet.column_dimensions[c.coordinate].width)
           
        # we now designate twidth (otal table width) as 90% \textwidth (leave 10% reserve)
        # maybe better: assume a textwidth of 80 letters, and the excel column width is also given in avg. letters (?)
        # so: total_tablewidth = twidth => twidth/80 is the fraction of the full tablewidth of \textwidth
        fractwidth = 1 / 80
        if twidth > 0.9*80:
            makesmall = "{\\small\\tabcolsep=3.5pt  % hold it local"
        else:
            makesmall = ""
            
        # a quick mapping of centering commands
        colalignmap = {'l': '\\raggedright', 'c': '\\centering', 'r': '\\raggedleft'}
        ...
        ###### TODO: It seems we need a complete rewrite of this code, it is highly fragmented and messy
        ### Should be possible to get all cell layout data in at most 2 passes (one for the width fraction, and 1 for the rest)
        ...
        if longtable:
            ### TODO: include default alignments, vertical borders and column widths
            # texout+= "\\begin{longtable}{@{}c@{}} \n" 
            col_align_str = "\\begin{longtable}{"
            ...
            for colnum in range(0, num_cols):
                ...
                colaligncode = e2lvp._pick_col_text_alignment(col2a)
                # quick & dirty, just use this result to make our "alternative" col def
                # of the form p{0.4\textwidth}>{\centering}
                # first comes the p (valign = top by default)
                ### TODO: vertical cell alignment from data
                col_align_str += "p"
                ...
                # ... now the width
                # nice reference here: https://texblog.org/2019/06/03/control-the-width-of-table-columns-tabular-in-latex/
                col_align_str += "{%.2f\\textwidth}"%(colwidth[colnum]*fractwidth)
                # and the alignment:
                #col_align_str += ">{%s}"%colalignmap[colaligncode]
